...
export interface CreateGameDiscoveryBatchV2Input {
  requestId?: string;
  userId: string;
...
export interface CreateGameDiscoveryBatchV2Result {
...
export async function createGameDiscoveryBatchV2(
  input: CreateGameDiscoveryBatchV2Input,
): Promise<CreateGameDiscoveryBatchV2Result> {
  ...
  const { data, error } = await service.rpc("orchestrator_create_game_discovery_batch_v2", {
    payload: {
  ...
  if (error) throw new Error(`Failed to create durable Game Discovery v2 batch: ${error.message}`);

  const row = object(data);
  const creativeRun = object(row.creative_run);
  if (typeof creativeRun.id !== "string" || typeof row.factory_job_id !== "string") {
    throw new Error("Invalid orchestrator_create_game_discovery_batch_v2 response");
  }
