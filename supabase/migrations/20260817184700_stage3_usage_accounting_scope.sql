-- Stage 3 / S3-005 accounting scope hardening.
-- Provider tasks predate the durable generation_image workflow and may also be used by
-- legacy content execution. Stage 3 image accounting must not relabel or zero-account
-- unrelated provider tasks, so both pricing and accounting triggers are fenced by the
-- owning factory job workflow_kind.
