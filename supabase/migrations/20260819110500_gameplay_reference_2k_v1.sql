-- Stage 4 gameplay references are human-reviewed widescreen gameplay evidence.
-- Request Nano Banana 2 at 2K so the 16:9 still is suitable as a desktop-game keyframe
-- before Kling animation. The exact downloadable gameplay video master is normalized
-- downstream by FFmpeg to 1920x1080.
...
BEGIN
  ...
  SELECT public.orchestrator_create_image_generation(jsonb_build_object(
      ...
      'settings', v_settings || jsonb_build_object('aspectRatio','16:9','effectiveQuality','2K','source_capture_format','desktop_pc_16x9','stage4_reference',true,'root_creative_run_id',v_root_run_id,'concept_id',v_concept_id,'moment_id',v_moment_id,'shot_id',v_shot_id,'gameplay_reference_lineage',v_reference_lineage),
  ...
  v_request_entry := jsonb_build_object('request_id',v_request_id,'generation_id',v_generation_id,'factory_job_id',v_child_job_id,'concept_run_id',v_concept.id,'concept_id',v_concept_id,'moment_id',v_moment_id,'shot_id',v_shot_id,'model_id',v_model_id,'aspect_ratio','16:9','effective_quality','2K',
