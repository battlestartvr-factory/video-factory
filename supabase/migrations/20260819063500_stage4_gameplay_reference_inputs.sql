-- Stage 4 gameplay reference inputs + lineage.
-- Drive remains the durable media archive; provider URLs are short-lived derived cache URLs.
...
BEGIN
  ...
  SELECT public.orchestrator_create_image_generation(jsonb_build_object(
      ...
      'settings', v_settings || jsonb_build_object(
        ...
        'shot_id',v_shot_id,
        'gameplay_reference_lineage',v_reference_lineage
      ),
  ...
  v_request_entry := jsonb_build_object(
    ...
    'shot_id',v_shot_id,
    'model_id',v_model_id,
    'gameplay_reference_ids',(
      SELECT COALESCE(jsonb_agg(item->>'referenceId'), '[]'::JSONB)
      FROM jsonb_array_elements(v_reference_lineage) item
    ),
    'gameplay_reference_lineage',v_reference_lineage,
    'created_at',NOW()
  );
  ...
  UPDATE public.creative_runs
  SET outputs=COALESCE(outputs,'{}'::JSONB)||jsonb_build_object('reference_image_request',v_request_entry)
