-- Stage 4 gameplay video primary route: MiniMax H3 / Hailuo 03 through KIE unified jobs.
-- Kling 3 remains enabled as a supported fallback/baseline.
...
VALUES (
  '20000000-0000-4000-8000-000000000045'::uuid,
  'kie', 'video', 'balanced', 'minimax-h3',
  'https://api.kie.ai/api/v1/jobs/createTask', true, 5,
  jsonb_build_object(
    'api_style', 'unified_jobs',
    'provider_model', 'minimax/hailuo-03',
    'durable_workflow', 'generation_video@1',
    'primary_gameplay_video', true,
    'default_duration_sec', 10,
    'default_resolution', '768P',
    'supported_duration_min_sec', 4,
    'supported_duration_max_sec', 15
  ),
  '{}'::jsonb
)
...
-- Keep Kling available. Older migrations enabled it already; this explicit update documents
-- that the H3 switch is a primary-route change, not a provider removal.
...
BEGIN
  ...
  v_video_model := COALESCE(NULLIF(v_concept.outputs#>>'{gameplay_shot,generationPlan,videoModel}',''),'minimax-h3');
  v_duration_sec := COALESCE(NULLIF(v_concept.outputs#>>'{gameplay_shot,generationPlan,durationSec}','')::INTEGER,10);
  IF v_video_prompt IS NULL THEN RAISE EXCEPTION 'video prompt is missing for approved shot'; END IF;

  IF v_video_model = 'minimax-h3' AND (v_duration_sec < 4 OR v_duration_sec > 15) THEN
    RAISE EXCEPTION 'MiniMax H3 gameplay duration must be between 4 and 15 seconds';
  END IF;
  ...
  v_video_settings := jsonb_build_object(
    'aspectRatio','16:9',
    'durationSec',v_duration_sec,
    'sound',false,
    'stage4_gameplay_video',true,
    'source_capture_format','desktop_pc_16x9',
    ...
    'video_revision_number',v_video_revision_number,
    'source_video_generation_id',v_source_video_generation_id,
    'revision_review_id',v_revision_review_id
  ) || CASE
    WHEN v_video_model = 'minimax-h3' THEN jsonb_build_object('resolution','768P')
    WHEN v_video_model = 'kling-3' THEN jsonb_build_object('effectiveQuality','pro')
    ELSE '{}'::JSONB
  END;
  ...
  RETURN v_result||jsonb_build_object('root_job_id',v_root_job_id,'concept_run_id',v_concept.id,'concept_id',v_concept_id,'moment_id',v_moment_id,'shot_id',v_shot_id,'approved_reference_generation_id',v_reference_generation_id,'video_revision_number',v_video_revision_number,'duration_sec',v_duration_sec,'video_model',v_video_model);
END;
$function$;

UPDATE public.deployment_schema_contract
SET schema_version='20260822170000', updated_at=NOW()
